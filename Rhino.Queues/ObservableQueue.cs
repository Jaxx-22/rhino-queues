...
{
    public interface INeedMessageNotification
    {
...
#pragma warning disable 420
    public class ObservableQueue : INeedMessageNotification
    {
        [ThreadStatic]
        ...
        public ObservableQueue(IPEndPoint endpoint, string path)
            : this(endpoint, path, new QueueManagerConfiguration())
        {
        }

        public ObservableQueue(IPEndPoint endpoint, string path, QueueManagerConfiguration configuration)
        {
        ...
        public void Start()
        {
            ...
            Observable.Defer<MessagePayload>(() =>
            {
                return Observable.Empty<MessagePayload>();
            });
        ...
        public IObservable<Message> Receive(string queueName)
        {
            return Receive(queueName, null);
        }

        public IObservable<Message> Receive(string queueName, string subqueue)
        {
            return Observable.Create<Message>(
                x =>
                {
                    ...
                    do
                    {
                        ...
                        lock (newMessageArrivedLock)
                        {
                            Monitor.Wait(newMessageArrivedLock, TimeSpan.FromSeconds(1));
                        }
                    } while (message == null);
                    x.OnNext(message);
        ...
        private void EnsureEnslistment()
        {
            ...
            if (CurrentlyEnslistedTransaction == Transaction.Current && Transaction.Current != null)
                return;
        ...
        private class MessageAcceptance : IMessageAcceptance
        {
            ...
            private readonly ObservableQueue parent;
            private readonly QueueStorage queueStorage;

            public MessageAcceptance(ObservableQueue parent,
